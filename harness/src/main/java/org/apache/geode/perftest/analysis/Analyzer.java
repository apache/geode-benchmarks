...
public class Analyzer {
  ...
  public static void main(String[] args) throws IOException {
    ...
    BenchmarkRunResult benchmarkRunResult = analyzer.analyzeTestRun(baselineResultDir, testResultDir);
    benchmarkRunResult.writeResult(new PrintWriter(System.out));
    /* throw exc if failed? */

    StringBuilder message = new StringBuilder();
    for (BenchmarkRunResult.BenchmarkResult benchmarkResult : benchmarkRunResult.getBenchmarkResults()) {
      for (BenchmarkRunResult.ProbeResult probeResult : benchmarkResult.probeResults) {
        if (probeResult.description.equals("average latency")) {
          if (probeResult.getDifference() >= 0.05) {
            message.append("BENCHMARK FAILED: ").append(benchmarkResult.name).append(" average latency is 5% worse than baseline.\n");
          }
        }
      }
    }
